 ...
 *
 * 🚨 **2026-09-01 — 위 넷 중 `STEPS`는 「안 죽었습니다」.** 학교 3단계가 들어왔는데도
 *    그 목록은 살아남았고, 🎯 자리·🧬 조립대가 빠진 채라 🗺️ 동네에 **「3 / 3」**이 떴어요 —
 *    화면이 *"만들기가 끝났다"*고 약속하고 실제로는 안 끝난 상태로, **아무도 안 보는 사이에**
 *    범민 님이 *"능력치 고르는 게 없어졌네…?"*로 겪었습니다 (101번 §1-4 · 102번 §1).
 *    ✅ 지금은 **`creation-test.js`의 C-0~C-4**가 이 자리를 지킵니다. 여기서는 계속 안 걸어요.
 *    ⚠️ **교훈 — 「곧 죽을 것」이라 검사를 미룰 때는, 죽었는지 확인하는 사람을 정해 두세요.**
 *    (나머지 셋은 실제로 죽었습니다: 절대 점수 밴드 → 편차 `d` · `CARDS` 3장 → 단계별 `deal` ·
 *     자리→동네 흐름 → 초등부 뒤로 이동)
 *
 * ─────────────────────────────────────────────────────────────────────────
 * 🔒 이 파일이 지키는 다섯
