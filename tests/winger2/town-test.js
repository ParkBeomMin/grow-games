...
async function runOffers(muts) {
   ...
   *    `showOffers`가 이미 감춰 놔서, 그대로 가면 **변이를 넣어도 증상이 없어요**
   *    (실측 확인). 📨 조기 제안(①-C)이 들어오면 `playedStage("m")`이 false인 채로
   *    제안 화면에 서는 경로가 생기고, 그때 이 줄이 진짜로 살아납니다.
   *    그래서 그 상태를 **미리 만들어 두고** 「돌아오는 길이 판단을 다시 하는가」를 봅니다. */
