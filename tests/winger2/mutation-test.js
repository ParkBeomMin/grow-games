...
}
{
  ...
  const mut = measureSpot(load(MUT.SPOT1));
  ...
  check(!(mut.fwG >= FW130_GOALS_MIN && mut.wgA >= WG130_ASSIST_MIN),
...
}
{
  ...
  const mut = measureSc(load(MUT.SC1));
  ...
  check(!(mut.drop >= CONCEDE_DROP_MIN && mut.gain >= TEAMGOAL_GAIN_MIN),
 ...
 * ══════════════════════════════════════════════════════════════ */
const AFTER_RATIO_MIN = 1.20;      // 문턱 (22번 확정 · V1 기준선 1.311±0.004 · 변이 1.01~1.04)
...
}
{
  ...
  const mut = measureFat(load(MUT.FAT_A), 5, FAT_N);
  ...
  check(!(mut.ratio >= AFTER_RATIO_MIN && mut.cards >= CARDS_MIN),
