 ...
 *
 * 🚨 **2026-09-02 — 승격 쪽 술어를 「블록 전부」로 조였습니다.**
 *    그날 실측이 **5.7%**로 내려와 승격 문턱 6.0%를 넘었는데, **20벌 블록이 7.0 · 5.0 · 5.0**이라
 *    문턱이 **블록 흩어짐 한가운데**에 서 있었어요 — 평균 하나로 판정하면 **고장이 아니라
 *    우연으로** 승격 신호가 뜹니다(이 파일이 회귀 쪽에서 이미 피했던 그 모양이에요).
 *    ✅ 그래서 **문턱은 그대로 두고**(0.06) 술어만 바꿨습니다:
 *       승격은 **블록이 전부** 문턱 아래일 때만 · 회귀는 **평균**으로 (빨리 잡아야 하니까요).
 *    🔑 값을 옮기면 「지금이 정답」이 되고, 술어를 조이면 「확실할 때만 움직인다」가 됩니다.
 *
 *    ⚠️ **그리고 이 값은 곧 다시 움직입니다.** 🧒 초1~3 아크가 들어와 학교 카드가 8 → 11장이
 *       되면 편차 밴드가 다시 넓어져요(designer: 중앙 47.8% → 41.7%). 아래 접촉률은
 *       출발 능력치를 타므로 **N = 11 뒤에 다시 재는 것이 맞습니다.** 그전에 승격하지 마세요. */
const LO_CAP = 0.16;
const LO_GOAL = 0.06;
