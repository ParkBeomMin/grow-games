...
/* ---------- ⑥ 🎮 미니게임 바로 해보기 ----------
 * 가을야구 4종(🎯 수싸움 · 🏃 홈 승부 · 🎯 코스 공략 · 🔥 힘겨루기)과 투어 3종
...
async function playground() {
  ...
  for (const want of ["mind", "dash", "course", "clash", "wave", "sync", "roar"]) {
  ...
  mg.set("pos", "batter");
  const asBat = ["mind", "dash", "course", "clash"].map(statLine);
  mg.set("pos", "pitcher");
  const asPit = ["mind", "dash", "course", "clash"].map(statLine);
  ...
  check(asBat[2] === asPit[2], `🎯 코스 공략만 시점과 무관하게 같은 능력치다 (${asBat[2]})`);
  ...
  mg.set("stat", 55);
  for (const id of ["mind", "dash", "course", "clash", "wave", "sync", "roar"]) {
