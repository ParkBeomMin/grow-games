...
for(const it of items){
  ...
  try{
    ...
    // ③ 우리 팀 동료의 출전 수가 내 출전 수를 넘으면 두 번 센 거예요
    const mates=(S.squads&&S.squads[S.group]||[]).filter(x=>!x.me);
    const overApps=apps>0?mates.filter(x=>(x.apps||0)>apps).length:0;
    ...
    console.log(`${bug?"❌":ok?"✅":"❌"} ${it.id.padEnd(22)} act ${apps<0?"없음(오프시즌)":apps+"경기"} · 옛 race ${hadRace?"있었음":"없음"} → 표 ${rank.length}줄 · 기록 ${withApps}명 (다른 클럽 ${otherRec}/${otherXI})${overApps?`  ← 동료 ${overApps}명의 출전 수가 내 출전 수를 넘어요(두 번 셌어요)`:bug?"  ← 다른 클럽에 기록이 거의 없어요":""}`);
    if(bug)bad++;
    if(!ok)bad++;
    w.close();
