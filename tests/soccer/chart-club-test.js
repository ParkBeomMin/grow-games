/* 경기 결과의 평점 순위표에 라이벌 '소속'이 실제로 찍히는지 본다.
 *
 * 증상: 내 줄만 클럽이 나오고 상위 5명은 전부 "-"였다.
 * 원인은 표를 그리는 쪽이 아니라 **행을 만드는 쪽**이었다.
 * fillRivals가 act.rivals[i].club을 제대로 채워도, 순위 행을 만드는 map이
...
/* 라이벌 8명 — fillRivals가 채워 놓은 뒤의 모습 그대로.
 * pop을 내 점수보다 확실히 높게 둬서 라이벌이 상위 5칸을 차지하게 한다. */
const RIVALS = [
...
check(!html.includes("리오"), "상위 5칸이 전부 라이벌이다 (내 줄이 안 섞였다)");

// ① 핵심 — 라이벌 소속이 그대로 찍힌다
RIVALS.slice(0, 5).forEach((r) => {
