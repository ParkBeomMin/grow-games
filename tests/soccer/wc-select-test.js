...
guard("⑤-b 깜짝 발탁", () => {
  ...
  if (s4.wc) {
    ...
    /* ⚠️ 한때는 대회 중 배지에도 "깜짝 발탁으로 승선했어요"를 남겼어요.
     * 그런데 뛰는 내내 그 얘기가 붙어 있으니 프로듀서가 빼자고 했습니다 —
     * 소집 카드에서 한 번 말하면 충분해요. 그 결정을 여기서 지킵니다. */
    const badge2 = WC.badgeHTML().replace(/<[^>]+>/g, " ");
    console.log(`   대회 중 배지 — "${badge2.replace(/\s+/g, " ").trim()}"`);
    check(!/깜짝 발탁/.test(badge2),
      "대회 중 배지에는 깜짝 발탁 얘기가 없다 — 뛰는 동안엔 뛰는 얘기만 해요");
    s4.wc = null;
  }
