...
## 1-b. 프로세스를 이름으로 일괄 종료하지 마세요

```bash
pkill -f node          # ❌
pgrep -x node | xargs kill   # ❌
killall node           # ❌
```

**같은 이름의 다른 프로세스까지 잡습니다.** ⚽ 더 윙어 II 작업 중 실제로 났어요 —
시뮬레이션을 정리하려고 `pgrep -x node`로 죽였는데 **세션의 MCP 서버가 함께 죽어**
그 세션 내내 복구가 안 됐습니다.

- 내가 띄운 것은 **PID를 기억했다가** 그것만 죽이세요
- 백그라운드로 돌릴 거면 `run_in_background`를 쓰고 `TaskStop`으로 멈추세요
- 정리가 꼭 필요하면 **파일 경로나 인자로 좁혀** 확인한 뒤에

> 이 컨테이너에는 MCP 서버·에이전트·도구가 전부 node로 돕니다.
> **"내 node"와 "남의 node"를 이름으로는 못 가릅니다.**
