...
`SALES_K`를 `0.72` → `1.2`로 잠깐 바꾸고 **`curve-test.js`**를 돌린다. 빨간불이 떠야 한다.

`SALES_K`에 반응하는 건 `curve-test.js`다. `concept-test.js`가 아니다 —
그쪽 11·12·13번은 컨셉끼리의 **상대 비교**라 계수를 어떻게 바꿔도 계속 통과한다.
계수를 지키는 건 목표 곡선(대상 확률)이고, 그 곡선을 재는 파일이 `curve-test.js`다.
`0.4`로 낮추는 반대 방향도 한 번 확인하고 되돌린다. 되돌린 뒤 전체 테스트를 다시 돌린다.
