      ...
      if (ok && st.league === 1 && st.proYear === YEARS && up.length && P.$("btn-transfer")) {
        add({
          ...
          check: "리그 11개 묶음과 카드가 좁은 화면에서 안 겹치고, 카드마다 리그·계약금·평점 페널티·나라 특색이 다 읽히는지. "
            + "카드마다 <b>🏟️ 전력 N위/6</b>이 붙어요 — 전력 숫자만으로는 '3부의 최강'과 '1부의 최약체'가 구분이 안 됐어요",
      ...
      if ((st.moves || []).length >= 3 && P.active() === "screen-career") {
        add({
          ...
          check: "소속 칸에 클럽 이름이 나오는지, 리그 태그가 붙은 칸이 세로로 쪼개지지 않는지 봐주세요. "
            + "위쪽 요약줄과 표의 소속 칸 아래에 <b>그 시즌 팀 최종 순위</b>가 함께 보여요.<br>"
            + "이번에 <b>팀 성적이 내 활약에서 떨어져 나왔어요.</b> 경기를 몇 판 치러 보면 "
            + "<b>내가 2골 넣고도 지는 경기</b>(약 21%)와 <b>한 개도 못 올렸는데 이기는 경기</b>가 나와요. "
            + "실점이 상대 클럽 전력을 따라 움직이니, 강팀을 만난 날은 스코어가 크게 벌어집니다",
...
function makeSoccerWc(kind) {
  ...
  for (const seed of seeds(kind === "live" ? 10 : 8)) {
    ...
    try {
      ...
      if (kind === "live" || kind === "lucked") {
        ...
        add({
          ...
          check: (lucked
            ...
            + "🎖️ <b>대표팀 안에서도 선발 경쟁이 있어요</b> — 경기 버튼에 <b>선발 확률</b>이 "
            + "적혀 있는지 봐주세요(🏅 국가대표 주전 92부터는 <b>선발 확정</b>이에요). "
            + "밀리면 🪑 벤치 카드가 뜨고 <b>누가 내 자리에 섰는지</b> 이름이 나와요. "
            + "그 경기는 <b>내 기록에 안 쌓이지만</b> 조 순위표와 개인 순위는 나아갑니다.<br>"
      ...
      if (kind === "luck") {
        const p = WC.luckP(ovr, BAR);
        if (!(p > 0.15 && ovr < BAR)) throw new Error(`깜짝 발탁 구간이 아니에요 (종합 ${ovr} · ${Math.round(p * 100)}%)`);
      }
      const wild = kind === "rookie", luck = kind === "luck";
      const P_LUCK = luck ? Math.round(WC.luckP(ovr, BAR) * 100) : 0;
      add({
        id: wild ? "soccer-wc-rookie" : luck ? "soccer-wc-luck" : "soccer-wc-invite",
        game: "soccer", url: "soccer/", emoji: wild ? "🌱" : luck ? "🎲" : "📨",
        ...
        steps: [
          ...
          "시즌 끝까지 진행해 <b>대표팀 소집</b>(또는 미발탁)까지 가 보기",
        ],
        keys: snapshot(P),
      });
      P.close();
    ...
    try {
      ...
      if (st.proYear >= 12 && st.activity && Array.isArray(st.activity.race)) {
        add({
          id: "soccer-veteran",
          game: "soccer", url: "soccer/", emoji: "🏷️",
          title: `노쇠 구간 — ${st.proYear}시즌 · 시즌 칭호`,
          state: `${st.group} · ${P.get("leagueOf")(st).name} · ${st.proYear}시즌 · 종합 ${Math.round(P.get("overall")())}`,
          check: "① <b>🎖️ 시즌 칭호</b> — 화면 위쪽 칩 줄이에요. 지난 시즌 성적으로 받아서 "
            + "이번 시즌 경기에만 붙어요(골 +15% 같은). 경기를 치러 보고, 시즌이 끝나면 결산에서 "
            + "<b>다음 시즌 칭호</b>가 새로 정해지는지 봐주세요 — 성적을 못 내면 사라져요.<br>"
            + "② <b>🏷️ 선수 클래스</b> — 소속 줄 끝의 종합 옆에 붙는 이름이에요(11단계, "
            + "🎖️국가대표 후보·🏅국가대표 주전 포함). 이건 경기에 관여하지 않고 "
            + "돈 줄의 <b>수당 ×배수</b>로 갚아요. 🛍️상점에서 장비를 사서 종합을 올리면 "
            + "그 자리에서 <b>칭호 승급</b> 로그가 떠요.<br>"
            + "③ 🥇 개인 순위를 펼치면 경쟁자마다 클래스가 붙어 있어요 — 리그가 높을수록 위쪽이 많아야 맞아요",
          steps: [
            "게임이 열리면 <b>이어하기</b> → 선수 카드",
            "화면 위 <b>🎖️ N시즌 칭호</b> 칩 줄과, 소속 줄 끝의 클래스 · 💰 줄의 <b>수당 ×배수</b>를 확인",
            "경기를 몇 판 치러 보고, 시즌 결산에서 <b>다음 시즌 칭호</b>가 새로 뜨는지 확인",
            "<b>🥇 개인 순위</b>를 펼쳐 경쟁자 클래스를 확인",
          ],
          keys: snapshot(P),
        });
        P.close();
 ...
 * 안 나왔어요.** 지금은 리그 전체에서 뽑아 동료가 소속 칸에 섞여 있어야 해요.
 *
      ...
      if (played >= MATCHES && ready && rosterOK && mates.length) {
        ...
        add({
          ...
          check: "① 준비 화면의 <b>🥇 개인 순위</b>와 경기 후 <b>평점 순위표</b>에 나오는 이름이 같은지, "
            + `② 소속 칸에 <b>우리 팀(${st.group})</b> 선수가 섞여 있는지, `
            + "③ 평점 숫자를 누르면 <b>⭐ 어떻게 나왔나</b>가 펼쳐지는지, "
            + "④ 중계에 <b>동료 이름으로 골</b>이 뜨면 그 선수 득점이 실제로 늘어나는지 봐주세요",
          steps: [
            "게임이 열리면 <b>이어하기</b> → 선수 카드",
            "준비 화면의 <b>🥇 개인 순위</b>를 펼쳐 이름을 봐 두세요 — 탭(⚽득점 🅰️도움 🛡️수비 🎯공격P ⭐평점 🏅MOM)마다 그 부문 숫자 하나만 보여요. 👑이 그 부문 1위(수상자)예요",
...
function makeSoccerEnding(kind) {
  ...
  add({
    ...
    title: kind === "youth" ? "유스 재계약 엔딩 — 🔥 1년 특훈" : "세미프로 입단 엔딩 — 프로 커리어 시작",
    state: `${best.info} — 프로 도전 직전`,
    check: kind === "youth"
      ? "엔딩 카드 아래 <b>🔥 1년 특훈 시작</b>이 맨 위에 오고, 버튼 3개가 세로로 안 겹치는지. "
        + "누르면 특훈 화면이에요 — 능력치를 고르고 <b>💪 집중 훈련</b>(3초 연타 레이어가 뜨는지, "
        + "친 만큼 오르는지)과 <b>🎲 무리한 특훈</b>(🌠 대박이 한 번씩 나오는지)을 여섯 회차 굴려 주세요. "
        + "🤖 경기 자동 진행을 켜 뒀어도 연타는 뜨고, 레이어 안의 <b>🤖 자동으로 맡기기</b>로 건너뛸 수 있어요"
      : "엔딩 카드에 K리그3가 적히고 <b>⚽ 프로 커리어 시작!</b> 버튼이 보이는지",
    steps: [
      "게임이 열리면 <b>이어하기</b> → 선수 카드",
      "육성 화면에서 <b>🔥 프로 도전 시작!</b>",
      "트라이아웃을 끝까지 진행하면 엔딩 카드가 떠요",
    ].concat(kind === "youth" ? ["<b>🔥 1년 특훈 시작</b> → 능력치 고르기 → 💪/🎲 여섯 회차"] : []),
    odds: {
