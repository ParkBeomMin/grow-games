    ...
    try {
      ...
      if (st.proYear >= 12 && st.activity && Array.isArray(st.activity.race)) {
        add({
          ...
          check: "칭호는 11단계예요(🪑벤치 → 🔄로테이션 → 🧢리그 주전 → 💪리그 정상급 → "
            + "🎖️국가대표 후보 → 🏅국가대표 주전 → 🌟월드클래스 → ⭐슈퍼스타 → 👑세계 최고 → "
            + "🐐살아있는 전설 → 🏆축구의 신). 소속 줄 끝의 <b>종합 옆 칭호</b>와 "
            + "돈 줄의 <b>수당 ×배수</b>를 봐주세요 — 이게 칭호의 효과예요. "
            + "훈련이나 🛍️장비로 종합을 올려 문턱을 넘으면 그 자리에서 <b>칭호 승급</b> 로그가 뜨고 "
            + "명성이 붙어요. 🥇 개인 순위를 펼치면 <b>경쟁자마다 칭호</b>가 붙어 있어요 — "
            + "리그가 높을수록 위 칭호가 많이 보여야 맞아요. "
            + "11시즌부터는 노쇠 구간이라 시즌 끝마다 능력치가 깎이고, 문턱 아래로 내려가면 칭호도 내려가요",
          steps: [
            "게임이 열리면 <b>이어하기</b> → 선수 카드",
            "소속 줄 끝의 칭호와 💰 줄의 <b>수당 ×배수</b>를 확인",
            "🛍️상점에서 장비를 사서 종합을 올려 보면 <b>칭호 승급</b> 로그를 볼 수 있어요",
