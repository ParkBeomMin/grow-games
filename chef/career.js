...
window.ChefCareer = (() => {
  ...
  function finishYear() {
    ...
    const awards = [];
    if (S.proYear === 1 && hype >= 3 && Math.random() < 0.8) { awards.push("신인셰프상"); S.career.rookie += 1; }
    if (hype >= 6.5 && Math.random() < 0.45) { awards.push("올해의셰프"); S.career.daesang += 1; }
    else if (hype >= 4.5 && Math.random() < 0.5) { awards.push("맛집선정"); S.career.bonsang += 1; }
  ...
  async function renderRanking() {
    ...
    $("battle-rank").innerHTML = `
      ...
      <table class="rank-table"><thead><tr><th>#</th><th>셰프</th><th>전적</th><th>레이팅</th></tr></thead>
