 ...
 *   💥 surge — 힘 배분. 한 판에 쓸 힘이 정해져 있고 세 승부에 나눠 쏟아요.
 *              상대의 힘은 숫자까지 다 보여요 — 이길 수 없는 줄을 **버려야** 이겨요
...
// 🎯 수싸움 · 🏃 홈 승부 · 💥 힘 배분 · 🔥 힘겨루기 결과 문구 (타자 / 투수)
...
const SURGE_BAT = { ok: "한 승부를 힘으로 가져왔어요! 💥", great: "힘을 몰아 쏟아 담장을 넘겼어요!! 🚀", bad: "힘이 골고루 모자랐어요… 범타" };
const SURGE_PIT = { ok: "한 타자만 내주고 이닝을 막았어요! 💪", great: "남은 힘을 완벽히 배분해 삼자범퇴!! 🫀", bad: "체력이 바닥나 줄줄이 내줬어요…" };
...
const POST_MECH = {
  ...
  /* 💥 힘 배분 — 한 판에 쓸 힘이 정해져 있고, 세 승부에 그걸 나눠 쏟아요.
   * 여기도 시점이 통째로 뒤집혀요.
   *   타자(aim: over) — 상대보다 **많이** 실어야 그 줄을 가져와요. 3줄 중 2줄이면 완벽.
   *   투수(aim: hold) — 같기만 해도 **버텨요**(동점은 투수 것). 대신 **한 줄도 안
   *                     내줘야** 완벽이에요. 동점의 주인과 문턱이 같이 뒤집혀요.
   * 능력치는 **체력**이에요 — 남은 힘을 어디에 쓸까가 곧 체력 관리니까요.
   * 타자·투수 둘 다 stamina를 봐요 (8종에서는 안 쓰이던 능력치예요). */
  surge: {
    stat: () => S.stats.stamina,
    txt: () => (S.pos === "batter" ? SURGE_BAT : SURGE_PIT),
    run: (box, tier, cb) => {
      const bat = S.pos === "batter";
      window.PostStage.surge(box, {
        label: bat
          ? "💥 승부처의 한 타석! 남은 힘을 어느 공에 쏟을까요"
          : "💥 만루 위기! 남은 체력을 어느 타자에게 쏟을까요",
        aim: bat ? "over" : "hold",
        rows: bat ? ["1구", "2구", "3구"] : ["1번", "2번", "3번"],
        zonePct: miniZone(S.stats.stamina),
        tier,
        msg: bat ? null : {
          /* 투수는 '내주지 않는 것'이 이기는 것이라 문턱과 동점의 주인이 뒤집혀요. */
          poolLabel: "남은 체력",
          foeLabel: "타자",
          meLabel: "내 구위",
          cue: "🧮 타자가 실은 힘은 전부 보여요 — 못 막을 타자는 버리세요",
          win: "💪 세 타자를 모두 막았어요! 삼자범퇴",
          even: "🏏 한 타자만 내주고 막았어요",
          lose: "❌ 체력이 골고루 모자랐어요… 줄줄이 내줬어요",
          timeup: "⏱️ 체력을 다 못 쓰고 이닝이 끝났어요",
          tip: "타자를 눌러 구위를 실어요 · <b>골고루 나누면 셋 다 모자라요</b> · 서두를 필요 없어요",
          readyTitle: "💥 힘 배분",
          readyLines: [
            "이 이닝에 쓸 <b>체력</b>이 있어요. 세 타자에게 그 힘을 나눠 쏟아요.",
            "<b>날아오는 공도, 제한 시간도 없어요.</b> 화면은 누르기 전까지 멈춰 있으니 천천히 세어 보세요.",
            "타자가 실은 힘(🔥)은 <b>숫자까지 다 보여요</b>. 타자와 <b>같기만 해도</b> 막아 내요.",
            "타자들의 힘 총합이 내 체력보다 커요. <b>못 막을 타자는 버려야</b> 나머지를 막을 수 있어요.",
            "<b>세 타자를 다 막으면</b> 삼자범퇴(완벽), 한 타자를 내주면 겨우 넘긴 거고, 두 타자면 무너져요.",
          ],
          readyShort: "못 막을 타자는 버리고 나머지를 꼭 막아요 · 세 타자를 다 막아야 완벽",
          /* ⚠️ readyPick·readyUndo에는 태그를 넣지 마세요 — 설명 칸에 그대로 들어가요. */
          readyPick: "타자 줄을 누르면 남은 체력에서 한 칸이 그 타자에게 실려요 — 언제 눌러도 결과는 같아요",
          readyUndo: "마지막으로 실은 한 칸을 되돌려요. 잘못 눌러도 되살릴 수 있어요",
        },
      }, cb);
    },
  },
...
function playPostMini(container, cb) {
  const mech = POST_MECH[pick(["mind", "dash", "surge", "clash"])];
  const txt = mech.txt();
