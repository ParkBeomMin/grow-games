...
const MARKETS = [
  ...
  {
    id: "em", name: "신흥국장", emoji: "🌏", tier: "이머징 마켓",
    debut: 0.55, growth: 1.18, spot: 1.1,
    desc: "인도·베트남 등 고성장 시장. 수익 폭이 크지만 환·정치 리스크가 커요",
  },
  {
    id: "coin", name: "코인장", emoji: "🪙", tier: "24시간 가상자산",
    debut: 0.5, growth: 1.28, spot: 1.25,
    desc: "잠들지 않는 극변동 시장. 한 방이 크지만 그만큼 잃기도 쉬워요 (가상 종목)",
  },
];
...
function maybeEvent() {
  ...
  const events = [
    ...
    () => {
      S.stats.mental = clamp(S.stats.mental + 2, 0, statCap("mental"));
...
const MOMENTS = {
  ...
  capital: {
    good: ["비중 조절로 리스크를 완벽히 관리! 💰", "분산 포트폴리오가 하락장을 방어했어요 ✨", "현금 비중을 지켜 기회를 잡았어요 🔥"],
    bad: ["한 종목에 몰빵할 뻔했어요 😬", "비중 관리가 살짝 흔들렸어요 💦"],
  },
};
...
function openHelp() {
  if (window.Help) window.Help.open("📈 더 인베스터 도움말", HELP_SECTIONS);
}
