...
window.W2Moment = (() => {
   ...
   *
   * 🔒 문턱은 **그 판의 천장에 붙어 있습니다**(`TIER`의 `ceil`). 눈에 보이는 등급과
   * 안 보이는 등급이 갈리지 않게 TIER 한 곳에서만 정합니다 — 판정 자체는
   * 여기가 안 만들어요(머리말 참고). 이건 **손이 얼마나 정확했나**의 등급입니다. */
  /* 🔗 **「빈 곳에 닿았는가」 — 절대 경계입니다.** 물리이지 등급이 아니에요.
   * 🔴 **상대화하지 마세요.** `s > 0`은 «창 안에 들어왔다»와 같은 말이고,
   *    그건 판이 어떻든 같은 뜻입니다.
   * 🔴 **0.35로 올리지도 마세요.** 0.35는 `loneJudge`의 값인데 그 함수는 실제 경기에서
   *    **한 번도 안 불립니다**(주석이 스스로 적어 뒀어요). 안 돌아가는 폴백의 값을 화면에
   *    그리면 **아무도 안 보는 규칙을 화면이 지키게** 됩니다. */
  const TIER_IN = 0;
   ...
   *             = 0.75 ÷ (1 − 6 ÷ 28.75)
   *
   * 🔴 **넷 중 하나라도 바뀌면 손으로 튜닝하지 말고 위 식으로 다시 계산해서** 적으세요.
   * 🔴 **난이도를 움직이려면 여기가 아니라 `ONE_WIN`입니다**(한 벌: `CELL_FLOOR`).
   * 🚨 **`ONE_WIN`·`CELL_FLOOR`와 같은 표에 두지 마세요** — 저 둘은 🎚️ 손잡이고
   *    이건 🔗 그 결과를 읽는 자예요(원칙 ⑨ — 성격이 다른 것을 나란히 두지 않기). */
  const PERFECT_OF_CEIL = 0.9478;

  /* 🔗 **화면 등급의 유일한 주인** (설계 122번 §2-4 · §7-2 · 125번 §1-3).
   *
   * 🔑 **「완벽」은 «그 판에서 손이 낼 수 있는 최선을 냈나»입니다.**
   *    `ceil`은 **판 시작(t = 0)에 한 번 잰 그 판의 천장**이에요(`runShotGrid`의 `boardCeil`).
   * 🔴 **매 프레임 다시 계산하면 안 됩니다** — 그러면 가장 밝은 칸을 고르기만 해도
   *    늘 「완벽」이라 **「빨리 차라」라는 시간 축이 통째로 죽습니다.**
   *
   * 🪦 **옛 절대문턱 `0.75`는 되돌리지 마세요.** 중립(`mul = 1`) 천장이 `1 − 6/23 = 0.739130`이라
   *    **문턱이 천장 위**에 있었어요 — 「완벽」이 손으로 얻는 게 아니라 🦶 **주발 쪽으로 열린 판을
   *    뽑아서** 났습니다(실측 주발 100.0% / 약발 0.0%). 그 증거가 `end(...)`의 `" 🦶 약발로!"`예요 —
   *    **쓰여 있는데 한 번도 못 뜨던 문자열**입니다(설계 125번 §1-1).
   *
  ...
  function ender(wrap, ctx, ceil) {
    return (s, line) => {
      if (ctx.done) return;
      ctx.done = true;
      const v = clamp(s, 0, 1), tier = TIER(v, ceil);
  ...
  function runShotGrid(container, ctx, gate) {
     ...
     * 🔒 **격자를 만든 직후에 둡니다** — 아래 `boardCeil`이 이 줄을 부르거든요. */
    const cellS = (c, a) => sOne(oneErr(c.x, a.best), a.tight * c.mul);

    /* 🏔️ **이 판의 천장 — 「손이 낼 수 있는 최선」입니다** (설계 125번 §1-3).
     * 🔒 **판 시작(t = 0)에 딱 한 번** 잽니다. 🔴 매 프레임 다시 계산하면 가장 밝은 칸을
     *    고르기만 해도 늘 「완벽」이 돼서 **「빨리 차라」라는 시간 축이 통째로 죽습니다.**
     * 🔑 🦶 주발/약발이 여기로 들어옵니다 — 약발 쪽으로 열린 판은 천장이 **낮고**,
     *    그래서 「완벽」의 문턱도 같이 낮아져요. **절벽이 아니라 창 길이의 차이**가 됩니다
     *    (주발 0.88초 ↔ 약발 0.47초 — 🦶 태그의 「－25%」가 그제야 참말이 돼요). */
    const boardCeil = Math.max.apply(null, cells.map((c) => cellS(c, oneAt(board, 0))));
    ...
    const end = ender(wrap, ctx, boardCeil);
    const t0 = nowMs();
    let urgent = false;
    ...
    const paint = (t) => {
      ...
      for (const c of cells) {
        ...
        /* 🎯 **판정 등급 셋을 그대로 그립니다** (설계 122번 §2-2 · 125번 §1-3).
         *      `hot` s ≥ 이 판의 천장 × 0.9478  「지금 누르면 완벽」
         *      `in`  s > 0                      「빈 곳에 닿아요」
         *      없음  s = 0                      「키퍼 정면이거나 너무 멀어요」
         * 🔴 **`hot`은 「지금 가장 좋은 칸」이 아닙니다** — **「지금 누르면 완벽인 칸」**이에요.
         *    판의 **앞 1/4**에만 뜨는 게 맞습니다(🦶 주발 판 0.88초 · 약발 판 0.47초).
         * 🔒 **문턱을 「자주 뜨게」 내리지 마세요** — 내리는 순간 표시와 판 끝 문구가 갈라집니다.
         *    주인은 위 `TIER`예요. 🔴 그리고 `PERFECT_OF_CEIL`은 난이도 손잡이가 아닙니다.
    ...
    onTap(goal, (e) => {
      ...
      /* 🔗 문턱을 여기 다시 적지 않습니다 — **주인은 `TIER`**예요(위 주석).
       * 화면의 `hot`·`in`과 **같은 자 · 같은 `boardCeil`**이라야 «표시가 곧 사후 설명»이 됩니다.
       * ✅ `" 🦶 약발로!"`는 **이 변경으로 처음 뜹니다.** 옛 절대문턱 0.75가 약발 판의
       *    천장(0.652)보다 위라 죽어 있던 문자열이에요 — 죽은 게 아니라 의도였습니다.
       * 🎖️ **알약 배지로 두릅니다** (director 131번 §5-1). 같은 화면에 「약발 −25%」 벌점표가
       *    떠 있어서, 맨 문장으로 붙으면 ***"약발로밖에"***로 읽혀요 — 성취를 그린 화면이
       *    **조건을 단 변명**처럼 읽히는 자리입니다. 🔒 CSS(`.w2m-win-weak`)가 **먼저** 들어와
       *    있습니다 — 반대 순서였으면 클래스는 붙는데 화면엔 아무 일도 안 일어나요.
       * 🔒 `ender`가 `p.innerHTML = line + …`으로 넣어서 `<b>`가 그대로 섭니다. */
  ...
  return {
    ...
    _t: { sOne, sBar, winMul, wideOn, loneJudge,
      ...
      /* 🔗 **종속값이라 손잡이 표(`K`) 밖에 둡니다** — 성격이 다른 것을 나란히 두지 않아요.
       * 🔴 **검사는 문턱(0.9478 · 중립 천장 0.739130)을 직접 적으세요.** 여기서 읽어 가면
       *    상수를 바꿔도 검사가 따라가서 안 잡힙니다. 이 값은 **「종속 관계 검사」**
       *    (`CELL_FLOOR`·`ONE_WIN`을 변이시키면 완벽 문턱이 따라 움직이는가)에만 쓰세요. */
      DEP: { PERFECT_OF_CEIL, TIER_IN } },
  };
})();
