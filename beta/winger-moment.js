...
window.W2Moment = (() => {
   ...
   *      5칸: 20.00 × 0.30 = 6.0   ·   **6칸: 16.67 × 0.30 = 5.0**
   *   그래서 **읽힘으로 칸 수를 먼저 정하고, 그 칸 폭 위에서 비(`ONE_WIN`)로 난이도를 맞추는**
   *   순서입니다(112번 §11-5 정정). 6칸으로 가면서 `ONE_WIN`이 23 → 22.5로 따라 움직였어요.
   *
   * 📮 **balancer에게** — 6칸에서 「능숙」 `E[s]`가 5칸 대비 **+0.046** 남습니다(116번 §3-3).
   *   바닥이 6.0 → 5.0으로 내려가 **천장이 올라간** 몫이라 `ONE_WIN`으로는 못 없애요
   *   (`ONE_WIN`은 셋을 같은 방향으로 밀 뿐이라 기울기가 안 바뀝니다).
   *   🔬 실측해 뒀습니다 — **`CELL_FLOOR = 0.36`**이면 바닥이 16.67 × 0.36 = 6.0으로
   *   **5칸과 같은 자리로 돌아오고**, 「능숙」 초과분이 +0.046 → **+0.010**으로 줄어요.
   *   🔴 **제가 안 바꿨습니다** — 확정 계수라 판정은 balancer 몫이에요. */
  ...
  const ONE = { cells: 6, off: [5, 9], slide: [5, 16], cov0: 10, cov1: 34,
    grow: 2400, life: 3400, look: 450, post: 3.5 };

  /* 🎚️ **난이도 손잡이 — 「속도 ÷ 창」의 창 쪽.** 여기 하나만 만지세요.
   * 「속도」는 좋은 지점이 밀리는 빠르기이고, 그 둘의 **비**가 난이도예요.
   * 🔴 `ONE_CLOSE`나 `CELL_FLOOR`로 난이도를 맞추지 마세요 — 그 둘은 다른 축입니다.
   *
   * 📏 **23 → 22** (116번 §3). 두 가지가 같이 움직여서 내려갔어요 —
   *      ① 칸이 5 → 6으로 좁아지며 `CELL_FLOOR` 바닥이 6.0 → **5.0**으로 낮아짐
   *      ② 🧤 키퍼가 **미끄러지기 시작**하면서 빈 곳이 덜 줄어듦
   *    둘 다 `s`를 올리는 쪽이라, 창을 그만큼 좁혀 되돌린 값입니다.
   *    🔒 **짝지은 차이로 잡았습니다**(옛 5칸·고정 키퍼 ↔ 새 6칸·미끄러지는 키퍼 ·
   *    같은 조작자 · 같은 판 · 400판 × 시드 3벌):
   *      「보통」 **+0.0012** (계약 ≤ 0.005) · 서툰 +0.013 · 능숙 +0.050
   *    🔴 **절대값을 계약으로 삼지 마세요** — 그건 조작자 모델의 성질이에요. 계약은 **차이**입니다.
   *    ⚠️ 22.5도 재 봤는데 「보통」이 +0.0089로 계약 밖이었습니다. */
  const ONE_WIN = 22;
  ...
  function runShotGrid(container, ctx, gate) {
    ...
    onTap(goal, (e) => {
      ...
      end(s, s >= 0.75 ? W.great + (ctx.weak ? " 🦶 약발로!" : "")
        : s > 0 ? W.ok
  ...
  const WORDS = {
    goal: {
      title: "🥅 일대일 슈팅",
      why: "🧤 키퍼가 미끄러진 <b>반대쪽</b> — 가장 밝은 칸으로 차요",
      ...
      keys: [{ name: "골문 칸", desc: "밝을수록 좋은 칸이에요. 기다릴수록 밝은 칸이 줄어요" }],
    ...
    assist: {
      title: "⚡ 컷백 연결",
      why: "🧤 키퍼가 비운 쪽으로 굴려 주면 동료가 밀어 넣어요",
      ...
      keys: [{ name: "문전 칸", desc: "밝을수록 동료가 닿는 자리예요. 기다릴수록 줄어요" }],
  ...
  function play(container, opts, cb) {
     ...
     * 🔴 부르는 쪽에서 이미 걸러 주는 게 맞지만(career.js·game.js·town.js), 한 입구라도
     *    빠지면 **수비 상황에 상대 골문이 뜹니다.** 그래서 여기서도 막아요. */
    if (o.kind === "defend") { done(judge(0.5), { s: 0.5, moment: o.moment || "block", weak: false }); return; }
