 ...
 *    전부 **다른 축의 손잡이**입니다. */
const CHILD_CAP_STEP = 1.5;   /* 🔒 쓸 수 있는 창이 **1.42~1.75뿐**이에요 (135번).
 *    아래는 🏃wg가(0.5% 미만이면 안 갈림), 위는 fw가 묶습니다 — 여유가 양쪽 8~9%.
 *    ⚠️ `BLEND_W`·`STAT_CAP`·`CHILD_TALENT` 짝·`FOCUS_W_HELD` 중 **하나만 움직여도 다시 재세요**
 *    (검사 `child-cap-test` K-10b가 그 넷의 지문을 붙잡고 있어요).
 *    🔴 **3의 배수로 만들지 마세요** — `Σq`가 늘 짝수라 우수리 되돌리기가 아무 일도 안 하고,
 *    그 줄을 지워도 **81조합 전부 증상이 0장**이 됩니다 (K-8 감도). */
