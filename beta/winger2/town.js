 ...
 *
 *   ⚠️ **순서가 바뀌었습니다** (2026-09-03 · 설계 130번 §2-1) — 🎯 자리가 🧒 초4(열한 살)
 *      뒤로 올라가면서 **초등부보다 앞**입니다. 옛 표는 자리를 초등부 「뒤」에 두고
 *      무대를 *"중학교 진학"*이라고 적고 있었어요.
 *
 *   | 단계        | 카드 | 📍 자리 | 미니게임        | 📨 걸린 것 | 무대 |
 *   | 🎯 포지션   | —   | ✅ 여기서 고릅니다 | —          | —          | 열한 살 · 5학년을 앞두고 |
 *   | 🏫 초등부   | 2   | 🔴 안 봄 | 대표 3종        | 조기 제안 (🇰🇷 항상 + fit) | 공터 · 아무도 안 봄 |
 *   | 🏫 중등부   | 3   | ✅      | `MINI[kind][pos]` 4종 | 조기 제안 (fit) | 학교 운동장 |
 *   | 🏫 고등부   | 3   | ✅      | 〃              | 🏟️ **최종 5곳** | 관중 · 스카우트석 |
 *   🔒 초등부의 「🔴 안 봄」은 **고르기 전이라서가 아닙니다** — 이제는 이미 골랐는데도
 *      `town.js`가 `ELEM_POS`("mf")로 덮어써요(§`deal`). 그 강제를 지우면 초등 미니게임
 *      분포가 갈려 `PEER_REF.town`과 편차 밴드를 통째로 다시 재야 합니다.
 *
...
window.WingerTown = (() => {
   ...
   *   🅰️ 전개 miss    → 역습 실점 (상대 +1) ·   🧱 수비 miss → 실점 (상대 +1)
   *   그 밖은 스코어가 안 움직여요
   *
   * 🔴 **`a.miss`는 2026-09-02에 새로 열었습니다** (inspector 118번 §6-1d).
   *    🧱이 `PLAYABLE`에서 빠지면서 `d.miss`가 **닿을 수 없는 줄**이 됐고, 그래서
   *    **학교 경기가 언제나 `N : 0`**으로 끝났어요 — 관중석이 찬 고등부 대회까지요.
   *    🔑 살아 있는 판정 중 **실점으로 읽히는 것은 「패스가 끊겼다」 하나**입니다
   *      (*"😖 발 끝에 안 걸렸어요"*는 슛 실패라 실점이 아니에요 — `g.miss`는 안 엽니다).
   *    🔒 **새 굴림이 0입니다** — 이미 나온 판정 `r`을 읽기만 해요.
   *    🔒 **`PTS`·`d`·`spotMul`에 한 톨도 안 닿습니다** — 스코어는 화면에만 갑니다.
   *    ⚠️ **되돌리는 법은 이 칸 하나를 지우는 것**입니다. 🧱 수비 격자가 돌아오면
   *       `d.miss`가 다시 살아나니, 그때 여기를 지울지 designer가 판정하세요.
   * ⚠️ `d` 칸은 **지금 안 쓰입니다**(🧱이 `PLAYABLE`에서 빠졌어요). 지우지 않는 건
   *    수비용 격자가 돌아오는 날 이 표가 그대로 서야 하기 때문이에요(117번 §6-4). */
  const GOAL_BY = { g: { perfect: "us" }, a: { perfect: "us", miss: "them" }, d: { miss: "them" } };
  ...
  const CALL = {
    ...
    a: { perfect: ["assist", "🅰️ {me}의 패스! 친구가 그대로 밀어 넣습니다!"],
      ...
      /* 🔴 끊긴 패스가 **역습으로 이어져 실점**합니다 (`GOAL_BY.a.miss` 참고).
       * 스코어가 움직이는 줄이라 `result`도 `concede`로 맞춰요 — 화면과 스코어가 어긋나면
       * *"점수는 올랐는데 아무 일도 없었다"*가 됩니다. */
      miss: ["concede", "😣 패스가 끊겨 역습… 실점"] },
  ...
  function openStage(stageId, opts, done) {
    ...
    function land(r) {
       ...
       * 🔴 여기서 상대 팀을 따로 굴리지 마세요 — 실점은 **내 카드의 `miss`**에서만 나옵니다
       *    (지금은 🅰️ 전개의 끊긴 패스 · 🧱 수비가 돌아오면 그것도). `GOAL_BY`가 그 표예요. */
