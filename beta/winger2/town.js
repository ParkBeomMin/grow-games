 ...
 *    🆕 **2026-09-04 — 🏁 「경기 시작」은 그 셋이 아닙니다**(§`kickoff`). 고를 게 없고,
 *    갈래가 없고, 세이브에 한 톨도 안 남아요. 그리고 **새 버튼이 아니라** 이미 있는
 *    `#btn-town-next`가 카드 0번까지 맡는 것이라, 화면의 조작은 여전히 **하나뿐**입니다.
 *    🔒 그 「하나뿐」이 위 셋을 막는 자리예요 — id를 하나 더 만들면 그 자리가 무너집니다.
...
window.WingerTown = (() => {
  ...
  const STAGES = [
    /* 🔴 `title`을 바꾸면 **두 곳이 따라옵니다** — `town.js:733`의 「N판을 마쳤어요」와
     *    `game.js:1680`의 「📨 …이 끝났어요」가 `stage.title`을 그대로 읽어요.
     * 🔒 `sub`는 중등부 것과 **한 벌**입니다 — 자리는 🧒 초4 뒤에 골랐는데
     *    `town.js:583`이 초5에서 `ELEM_POS`("mf")로 덮어씁니다. 덮는 걸 감추지 않고
     *    **말해 버려서 계단으로** 만들었어요: 초5 「정했는데」 → 중등 「이제 정말」.
     * 🔒 `next`가 「제안」을 약속해도 되는 근거: `fitFor`가 `at === 0`이면 `FIT_HOME`을
     *    무조건 밀어 넣어(`:532`) **초5 뒤에 손 든 곳이 0곳일 수 없습니다.**
     *    ⚠️ 중등부는 0곳일 수 있어 `m.next`는 「제안」을 약속하지 않습니다. */
  ...
  function openStage(stageId, opts, done) {
    ...
    let i = 0;
    /* 🏁 **이 함수를 화면에 들어서면서 바로 부르지 않습니다** — 아래 `kickoff()`가
     *    [🏁 경기 시작]을 한 번 받고 나서 부릅니다(2026-09-04 · 범민 님). 근거는 거기 있어요. */
    function playCard() {
      const c = deck[i];
    ...
    /* ══════════════════════════════════════════════════════════════
     * 🏁 **경기 시작** — 화면에 들어서자마자 굴리지 않습니다 (2026-09-04 · 범민 님)
     * ══════════════════════════════════════════════════════════════
     * *"초5되자마자 바로 시작되네 그러지말고 경기 시작 버튼 누르면 경기가 시작되도록"*
     *
     * 🔑 **새 버튼이 아닙니다.** 이미 있는 `#btn-town-next`가 카드 **0번**까지 맡는 것이에요 —
     *    그래서 🏫 학교 화면의 조작은 여전히 **하나뿐**입니다(`town-test` T-7a).
     *    🔴 별도 id로 버튼을 하나 더 만들지 마세요. 그 순간 «건너뛰기·난이도·재도전을
     *       붙이지 마라»를 지키는 자리가 「버튼이 둘이어도 되는 화면」이 됩니다.
     *
     * 🔒 **세 「경기」의 첫머리에 한 번씩입니다 — 카드마다가 아니에요.**
     *    초·중·고는 각각 **다른 경기**입니다(스코어가 단계마다 0에서 시작해요 · `hg`/`ag`).
     *    카드와 카드 사이는 **이미** [다음 판]이 있었으니, 버튼이 없던 자리는
     *    **각 경기의 첫 카드 하나**뿐이었어요. 여기를 메우면 아크 전체가
     *    「누르면 진행」으로 고르게 됩니다 (탭 8 → 11).
     *    🔴 **카드마다 「시작」을 붙이지 마세요** — 그건 8번이고, 진행 버튼과 뜻이 겹칩니다.
     *
     * ── 🔑 「[다음]은 한 번뿐인 결정에만」과 안 부딪히는 이유 (designer 133번) ──
     *   그 규칙이 막은 것은 **결정**입니다(✏️ 이름·🦶 주발·🗺️ 지도·🎯 자리 — 고르면
     *   세이브에 남고 갈래가 생기는 것들). 🏁 경기 시작은 **고를 게 없고, 갈래가 없고,
     *   세이브에 한 톨도 안 남습니다.** 해마다 오는 🧒 초1~초4에 [다음]을 안 붙인 근거
     *   («확정 탭이 해마다 붙으면 결정이 두 배»)가 여기엔 안 걸려요.
     *
     * ── ⏱️ 계약 D(첫 순간 카드까지 ≤ 90초)에 걸리는 것은 **초5 하나뿐**입니다 ──
     *   D는 **첫** 카드까지만 잽니다. 🏫 중등·고등의 킥오프는 첫 카드 **뒤**라
     *   D에 한 톨도 안 닿아요. 🔑 그래서 «어디까지 붙이나»는 **D의 문제가 아닙니다** —
     *   1곳을 붙이든 3곳을 붙이든 D가 무는 값이 **똑같습니다.**
     *
     * 🔒 **`pointerdown`이 아니라 `click`입니다**(원칙 ⑥). 손을 뗄 때 브라우저가
     *    그 지점의 새 요소로 `click`을 보내서 **즉시 두 번 먹히는** 자리예요 —
     *    미니게임 준비 화면에서 실제로 났던 버그입니다. `land()`의 [다음]과 같은 자리(`onclick`)를 씁니다.
     * 🔴 **버튼이 없으면(마크업이 낡았으면) 옛 동작 그대로 바로 굴립니다** — 스크립트
     *    하나가 안 왔다고 선수 만들기가 통째로 멈추면 안 돼요(이 파일의 다른 폴백과 같은 자리). */
    function kickoff() {
      if (!btn) { playCard(); return; }
      /* 🔒 앞 단계의 자국을 여기서 지웁니다 — 예전엔 `playCard`가 지웠는데, 이제 그 사이에
       *    이 화면이 끼어서 🏫 중등에 들어설 때 **초5의 결과 줄이 그대로** 서 있어요. */
      if (hint) hint.innerHTML = `<b>🏁 킥오프</b> — 90분 동안 순간이 <b>${deck.length}번</b> 와요.`;
      if (prog) prog.innerHTML = progHTML(deck, 0);
      if (res) res.innerHTML = "";
      if (card) { card.innerHTML = ""; card.className = "w2m-town"; }
