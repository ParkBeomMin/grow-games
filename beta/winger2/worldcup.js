...
window.WingerWorldCup = (() => {
   ...
   *
   * 두 가지가 문턱을 움직여요.
   *   ① 국가 전력 — 강한 나라일수록 앞에 선 사람이 많아 문이 좁고, 약체면 넓어요.
   *      ⚠️ **대표팀이 🇰🇷로 고정된 뒤로 이 항은 상수예요** (−3.33 · 문이 넓은 쪽).
   *      예전에는 유스 국적이 여기 걸려서 "강한 나라는 대회에서 유리한 만큼
   *      들어가기가 어렵다"는 고리를 만들었는데, 그 고리를 **의도적으로 뺐습니다**
   *      (아래 MARKET_NATION 주석에 왜 뺐는지 적어 뒀어요). 산식은 그대로 둬요 —
   *      되돌릴 자리를 지우지 않으려는 것입니다.
   *   ② 세대 — 대회마다 ±2쯤 흔들려요. "이번 세대는 유난히 두껍다"가 생깁니다.
   *      ⚠️ 🇰🇷 고정 뒤로는 **문턱을 실제로 움직이는 게 이것 하나뿐이에요.**
   *
   * ⚠️ 흔들림은 **무작위가 아니라 시즌·국가에서 뽑은 고정값**이에요. Math.random을
   * 쓰면 준비 화면을 다시 그릴 때마다 문턱이 달라져서 배지에 적힌 숫자를 믿을 수
   * 없게 됩니다 — 👥 선발 확률에서 이미 같은 사고가 났어요.
   * 배지에 **나라 이름을 같이** 적는 건 이제 다른 이유예요 — 🇰🇷 고정 뒤로 문턱이
   * 달라지는 건 ② 세대뿐이라 나라 이름이 그 차이를 설명하지는 않습니다. 대신
   * "누구의 문턱인지"를 적어 두는 자리로 남겨요. 유스를 해외로 골랐어도 대표팀은
   * 🇰🇷라는 걸 화면이 한 번은 말해 줘야 하거든요. */
  const BAR_NAT_K = 0.5;      // 국가 전력이 문턱에 실리는 정도 (🇰🇷 고정이라 지금은 상수 −3.33)
  ...
  /* 🇰🇷 내 대표팀은 **언제나 대한민국이에요 — 유스를 어디로 골랐든 고정입니다.**
   *
   * 예전에는 유스 국적(S.market)이 대표팀을 정했어요. 아래 MARKET_NATION이 그
   * 표인데 **지금은 어디에서도 쓰지 않습니다.** 지우지 않고 남긴 건 되돌릴 근거를
   * 지우지 않으려는 것뿐이에요 — 🔴 **되살리지 마세요.** 되살리면 아래 "잃은 고리"를
   * 그대로 되돌리는 겁니다.
   *
   * 왜 고정했나 — 🏘️ 동네에서 시작해 🗺️ 출신이 한국 17개 시·도가 되는데, 브라질
   * 유스를 골랐다고 🇧🇷 대표로 월드컵을 뛰면 앞뒤가 안 맞아요. 해외 유스에서
   * 자라 자국 대표로 뛰는 건 실제 축구 그대로고요. 그리고 항상 약체로 시작하니
   * **"내가 한국을 우승시킨다"**가 됩니다 — 강한 나라를 골라 편하게 가는 쪽은
   * 언더독 서사를 스스로 없애는 선택이었어요.
   *
   * 🔴 **의도적으로 뺀 고리**: 국적 트레이드오프(강한 나라 = 대회 유리 + 소집
   * 어려움)를 일부러 없앴습니다. 유스 선택이 유불리 하나로만 기울지 않게 하던
   * 고리였어요. 재 보니 폭이 작습니다 — 🌍 국가 전력 폭 4.90 · 국가 문턱 폭 7.00에
   * 비해 🌟 에이스 보정 폭이 14.00으로 **국가 폭의 2.9배**예요. 게다가 월드컵은
   * 프로 3시즌차부터 4년에 한 번이고 3시즌 도달률이 5~20%라(위 wildOpen 주석),
   * **커리어의 80~95%에서 그 축은 한 번도 발동하지 않았습니다.**
   * 🇮🇹·🇧🇷 유스는 대회 전력을 잃는 만큼 소집 문턱을 얻어요(−3.16 잃고 −4.50 얻음) —
   * **순손실이 아닙니다.**
   *
   * 🔒 그래서 NAT_SPREAD·BAR_NAT_K는 **그대로 둡니다.** 둘 다 "국가 **간** 폭"
   * 손잡이인데, 내 나라가 하나로 고정된 뒤에는 그걸 만지면 상대들의 폭과 함께
   * **내 위치가 같이 움직여요** — 「폭 손잡이」라는 이름으로 「위치」를 옮기게 됩니다.
   * 월드컵이 너무 어려우면 손댈 곳은 🌟 ACE 쪽이에요(팀 전력에 실리는, 유일하게
   * 남은 같은 축의 손잡이).
   *
   * 💾 옛 세이브는 **마이그레이션하지 않아요.** 🇧🇷로 저장된 커리어도 여기가 S를
   * 아예 안 보니 다음에 열 때 자연히 🇰🇷가 됩니다. 명예의 전당 항목에는 국적 칸이
   * 없어서(리그 나라 country만 있어요) 옛 헌액의 국기 표시도 바뀌지 않아요. */
  // 📕 쓰지 않는 표예요 — 되돌릴 근거로만 남겨 뒀습니다 (위 주석)
   ...
   *
   * 원래 이유: 표의 74~90을 그대로 쓰면 유스 국적이 성적을 통째로 정했습니다.
   * 실측(대회 6,000판) 종합 100에서 🇧🇷 브라질 우승 18% · 🇰🇷 한국 5% — **3.6배**.
   * 0.35로 좁히니 1.6배가 됐어요(목표 2.0배 이내). 순위 느낌은 남고 벽은 사라집니다.
   * 표를 직접 고치지 않고 여기서 좁히는 이유: "실제 전력을 이만큼 눌러 쓴다"는
   * 의도가 코드에 남아야 나중에 되돌리거나 다시 재기 쉬워요.
   *
   * 🔒 **🇰🇷 고정 뒤에는 이 값을 밸런스 손잡이로 쓰지 마세요.** 지금도 상대 국가들의
   * 폭을 좁히는 일은 하지만, 내 나라가 하나뿐이라 이 값을 움직이면 **내 전력
   * 위치까지 같이 움직입니다**(0에 가까울수록 🇰🇷가 평균으로 올라가요).
   * 「폭」이라는 이름으로 「위치」를 만지는 셈이라, 대회 난이도를 조정할 자리는
   * 아래 🌟 ACE 쪽입니다 — MARKET_NATION 주석 참고. */
  ...
  function openInvite(wild) {
    ...
    /* 🇰🇷 "브라질 유스를 골랐는데 왜 한국 대표지?"는 반드시 나옵니다. 그 질문이
     * 처음 생기는 자리가 여기라 여기서 한 번 말해요 — 유스 선택 화면은 월드컵이
     * 있는 줄도 모르는 시점이라 거기 적으면 그냥 소음이에요.
     * 국내 유스에게는 안 적어요. 당연한 말이라 소집의 순간만 흐려집니다. */
