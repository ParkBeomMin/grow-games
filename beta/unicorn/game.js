/* 더 유니콘 🦄 창업 방치 — 클릭→자동화→성장→Exit 프레스티지 */
"use strict";
...
function renderLog() {
  $("ev-log").innerHTML = S.log.map((l, i) => `<div class="${i === 0 ? "new" : ""}">${l}</div>`).join("");
}

// ---------- 클릭 ----------
function onClick(e) {
  const v = clickValue();
  S.money += v; S.earnedRun += v; S.earnedAll += v;
  floatText(e, "+" + fmt(v));
}
function floatText(e, txt) {
  ...
  const x = (e && e.clientX ? e.clientX - r.left : r.width / 2) + (Math.random() * 30 - 15);
  f.style.left = x + "px";
...
function buyUp(u) {
  if (S.ups[u.id] || S.money < u.cost) return;
  S.money -= u.cost;
  S.ups[u.id] = 1;
...
function doExit() {
  ...
  if (!confirm(
    ...
    `· 자금·직원·업그레이드는 초기화되고, 더 빠르게 다시 시작해요\n\n진행할까요?`
  )) return;
  S.so += gain;
  S.exits += 1;
  S.money = 0; S.gens = {}; S.ups = {}; S.earnedRun = 0;
  S.buffUntil = 0;
...
function renderHud() {
  ...
  $("hud-sec").textContent = fmt(perSec()) + " /초";
  $("hud-so").textContent = "🧾 스톡옵션 " + S.so + " (×" + prestigeMult().toFixed(2) + ")";
  $("stage-name").textContent = `${st.emoji} ${st.name}`;
  $("stage-val").textContent = "밸류에이션 ₩" + fmt(v);
  const bar = $("stage-bar");
  if (ns) {
    const lo = st.v, hi = ns.v;
    const pct = Math.max(0, Math.min(100, ((v - lo) / (hi - lo)) * 100));
    bar.style.width = pct + "%";
    $("stage-next").textContent = `다음: ${ns.emoji} ${ns.name} (₩${fmt(hi)})`;
  } else {
    bar.style.width = "100%";
    $("stage-next").textContent = "🏆 최종 단계 도달!";
  }
  const buffOn = Date.now() < S.buffUntil;
  $("buff-tag").classList.toggle("hidden", !buffOn);

  const exitBtn = $("btn-exit");
  if (v >= EXIT_UNLOCK) {
    exitBtn.disabled = false;
    exitBtn.innerHTML = `🚀 Exit (매각/IPO) — 스톡옵션 +${exitSO()}`;
  } else {
    exitBtn.disabled = true;
    exitBtn.innerHTML = `🔒 Exit — 시리즈 A(₩${fmt(EXIT_UNLOCK)})부터`;
  }
}
    ...
    return `
      <button class="gen ${can ? "" : "no"}" data-gen="${g.id}">
        ...
        <span class="gen-info">
          ...
          <span class="gen-desc">${cnt ? `${fmt(out)}/초` : g.desc}</span>
        </span>
        <span class="gen-cost">₩${fmt(cost)}</span>
      </button>`;
  }).join("");
  box.querySelectorAll(".gen").forEach((b) => {
    b.onclick = () => buyGen(GENERATORS.find((g) => g.id === b.dataset.gen));
  });
}
  ...
  box.innerHTML = avail.map((u) => {
    ...
    return `
      <button class="up ${can ? "" : "no"}" data-up="${u.id}">
        ...
        <span class="up-cost">₩${fmt(u.cost)}</span>
      </button>`;
  }).join("");
  box.querySelectorAll(".up").forEach((b) => {
    b.onclick = () => buyUp(UPGRADES.find((u) => u.id === b.dataset.up));
  });
}
...
function tick() {
  ...
  if (gain > 0) { S.money += gain; S.earnedRun += gain; S.earnedAll += gain; }
  // 밈 이벤트 (버프 중이 아니면 가끔)
  if (now >= S.buffUntil && Math.random() < 0.006 && baseSec() > 0) {
    S.buffUntil = now + 15000;
    addLog(MEMES[Math.floor(Math.random() * MEMES.length)] + " (15초간 생산 2배!)");
  }
  renderHud();
...
function offlineReward() {
  ...
  const earn = perSec() * elapsed * 0.5; // 오프라인은 50% 효율
  if (earn < 1) return;
  S.money += earn; S.earnedRun += earn; S.earnedAll += earn;
  const mins = Math.round(elapsed / 60);
  addLog(`💤 자리를 비운 ${mins}분 동안 ₩${fmt(earn)}를 벌었어요! (오프라인 50%)`);
  setTimeout(() => alert(`💤 자동 개발 완료!\n\n자리를 비운 ${mins}분 동안\n₩${fmt(earn)} 를 벌어뒀어요. (오프라인 50% 효율)`), 300);
}
