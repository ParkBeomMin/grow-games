...
/* 🥇 개인 순위 경쟁자 — 시즌 내내 같은 8명이 골·도움·수비를 쌓아요.
 *
 ...
 *
 * ⚠️ 라이벌(act.rivals)과 다른 명단이에요. 라이벌은 **반기마다** 다시 뽑혀서
 * 주간 평점 순위를 겨루는 8명이고, 여기는 **시즌 내내** 같은 8명이에요 —
 * 득점왕 경쟁은 시즌 단위라 중간에 사람이 바뀌면 표가 말이 안 됩니다.
 *
 * 경쟁자 생산량은 **리그 격을 따라가요.** 하부 리그에서는 상을 쓸어 담지만
 * 값어치(prestige)가 작고, 상위 리그에서는 득점왕 하나가 어렵습니다 —
 * 이 게임이 원래 갖고 있던 "쉬운 상 vs 값진 상" 축을 그대로 씁니다. */
const RACE_ROLES = [
...
/* 한 경기 기대치. 명성(pop 52~88)과 리그 격이 함께 실려요.
 *
 * 계수는 실측으로 잡았어요 (능력치 100 공격수 · 38경기 · 1200시즌 골든부츠 수상률):
 *   0.62 + 0.50×격 → K3 100% · K1 99%   ← 경쟁이 아니라 그냥 받는 상이었어요
 *   1.35 + 0.40×격 → K3  84% · K1 64%   ← 지금
 ...
 *
 * 눈금은 종합(스탯 평균, 상한 130)이에요. 경쟁자는 pop(52~88)으로 굴러서 자가
 * 다른데, 실측으로 맞췄습니다 — "pop 70 경쟁자 8명과 대등해지는 종합"을 리그마다
 * 재면 K리그3 55.7 · K리그1 63.4 · 챔피언십 97.8 · 프리미어리그 117.5가 나와요.
 * 여기에 직선을 맞춘 게 raceStr입니다 (최대 오차 5).
 * 그래서 프리미어리그 경쟁자는 대부분 🌟 월드클래스 위, K리그3 경쟁자는
 * 🧢 준수한 주전 아래에 놓여요 — 리그를 옮기면 표의 눈높이가 실제로 달라집니다. */
/* 문턱은 프리미어리그 경쟁자 분포(pop 52~88 → 실력 87~148)를 보고 잡았어요.
 * 🐐가 140이면 여덟 명 중 한 명쯤 나옵니다 — 130으로 두면 매 시즌 두세 명씩
 * '살아있는 전설'이 나와서 말이 값을 잃어요.
 * 내 쪽에서 🐐는 스탯 상한(130)을 넘겨야 하니 초월까지 가야 닿는 자리입니다. */
const PLAYER_TITLES = [
  [140, "🐐 살아있는 전설"],
  [118, "👑 세계 최고"],
  [95, "🌟 월드클래스"],
...
const titleOf = (power) =>
  (PLAYER_TITLES.find(([bar]) => power >= bar) || PLAYER_TITLES[PLAYER_TITLES.length - 1])[1];
